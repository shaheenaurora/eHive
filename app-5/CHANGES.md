...
## Interactive layer (apps.css + apps.js)
- **eHive guide (concierge)** — floating gold button on every page opens a scripted dialogue
  (typing indicator, quick-reply chips) that routes visitors to the right door. Honestly
  labeled "a scripted guide, not a bot pretending to think." Hidden on the portal.
- **30-second pathfinder** — modal quiz (3 questions, animated transitions, progress bar,
  confetti result card) recommending a door; triggers on the homepage hero, final CTA, and
  from the concierge.
- **Command palette** — Ctrl/Cmd+K anywhere: fuzzy quick-nav to every page, tool and action.
- **Hive Score simulator** (Circle) — action buttons animate a score ring; milestones unlock
  with toasts + confetti; demo mechanics, honestly labeled.
- **Setup journey walkthrough** (Business Setup) — 5-step interactive stepper with animated
  progress fill and a Play auto-advance mode.
- **Jurisdiction comparer** (Business Setup) — free zone vs mainland toggle with animated
  bars and swapping verdict labels; qualitative and honestly disclaimed.
- **Sprint day explorer** (Consulting) — hour-by-hour timeline with animated cards and Play mode.
- **Motion upgrades** — funnel steps slide in, calculator totals swap-animate, selector
  results pop, shared toast system; everything honors prefers-reduced-motion.
